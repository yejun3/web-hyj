...
-- 서브쿼리로 조회된 결과에는 이름이 없기 떄문에     
-- 서브쿼리로 조회된 테이블에는 별칭이 필수이다.

-- 부서별 평균을 inline view를 활용하여 조회하기
-- 출력컬럼 : department, avg_salary

select department, avg_salary
from(select department, floor(avg(salary)) as avg_salary
       from employee
       group by department
     ) as s; 

-- 평균급여가 5000 이상인 부서만 조회하세요
-- inline view 사용
...
-- 지역 : region
select*
from(select region,count(*) as region_sales
       from sales
       group by region
    ) as t;
...
from(select region, max(sales_amount) as  max_price, min(sales_amount) as min_price
      from sales
      group by region
     ) as f;
...
-- s.id는 1,2,3  e.student_id는 1,1,2 이기 떄문에 id가 3인 데이터 없음   id가 3인 민수는 조회되지 않는다.
-- on where 역할은 한다.
-- on절로 연결 기준을 알려줘야 한다.
-- 값 자체가 연결 역할을 하기 떄문에 fk로 설정하지 않아도 가능은 하다.
...
select name, class_name
from student s -- 전체테이블을 보고싶은 쪽을 from절에 쓴다.
left outer join enroll e 
on s.id = e.student_id;

-- 민수는 수강 내역이 없지만, 왼쪽 테이블에 있으므로 반드시 나온다.
-- 대신 enroll쪽 값은 없기 떄문에 null로 채워진다.
...
-- CROSS JOIN(곱집합)
-- 조건 없이 모든 조합을 다 만들어 버리는 JOIN
-- 색상과 사이즈의 모든 조합을 만들어보자
select c.name,e.name 
from color c
cross join size e;

-- 일반적인 조회에서는 많이 사용하지는 않으나, 모든 조합이 필요한 경우 사용
...
-- 추천 관계문장 출력하기
-- ex) 이서연의 추천인은 김민준
select concat(c.customer_name,'의 추천인 ', r.customer_name) as 추천관계
FROM customer c
INNER JOIN customer r
ON c.recommender_id = r.customer_id;
...
select * 
from customer_rental_count_view
where rental_count>=30;

-- view를 사용하면 좋은 점
-- 1. 복잡한 join과 조건이 반복될 떄
-- 2. 비즈니스 규칙을 숨겨둘 때
-- 3. 보안,권한 제어용으로 사용한다.
